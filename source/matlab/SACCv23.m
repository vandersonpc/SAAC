...
while 1
   finput = input(' Arquivo dos dados de Entrada *.dat [saccps.dat] => ','s');
   if isempty(finput)
      finput = 'saccps.dat';
   else
      finput = [finput,'.dat']; 
   ...
   fid = fopen(finput);
   if fid == -1
      fprintf(1,'\n Error!Arquivo de entrada nao existe!\n\n');
   else
      fclose(fid); 
      if isSACC(finput) == -1 
         fprintf(1,'\n Error!Arquivo de entrada nao e um arquivo SACC!\n\n');
      else
         break;
      end   
   end   
...
foutput = input(' Arquivo dos dados de Saída *.dat [saccout.dat] => ','s');
if isempty(foutput)
   fout = 'saccout';
 else
   fout = foutput; 
...
outdir = input(' Diretorio destino [Default] => ','s');
if isempty(outdir)
   outdir = 'Default';
end
   outdir = upper(outdir);

Zf = input(' Impedancia de falta em ohms [0] => ');
if isempty(Zf)
   Zf = 0;
end
fprintf(1,     '\n');
fprintf(1,     '+----------------------------------------+\n');
fprintf(1,     ' 1 - ANALISAR FLUXO DE POTENCIAS          \n');
fprintf(1,     ' 2 - V PRE FALTA DEFINIDA PELO USUARIO    \n'); 
fprintf(1,     '+----------------------------------------+\n');
metodo = input(' Escolha sua opcao [2] => ');
...
if metodo ~= 1
   Vibus = input(' Tensao de pre falta em pu [1] => ');
   if isempty(Vibus)
      Vibus = 1;
   end
end
...
fprintf(fid,'  No BARRAS: %d                                          \n',nbus);
fprintf(fid,'  No GERADORES: %d                                          \n',ngen);
fprintf(fid,'  No TRANSFORMADORES: %d                                          \n',ntf);
fprintf(fid,'  No LINHAS DE TRANSMISSAO: %d                                          \n',nlt);
fprintf(fid,'  No CARGAS: %d                                          \n',nld);
...
cagain = input('Deseja novo calculo?(S/N) [N]: ','s');
if isempty(cagain)
   cagain = 'N';   
end
cagain = lower(cagain);
